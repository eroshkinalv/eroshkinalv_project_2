    ...
    for vacancy in vacancies_info:
        vacancies_list.append(
            Vacancy(
                vacancy["name"],
                vacancy["url"],
                f"{vacancy['salary']['from'] if vacancy.get(
                                          'salary') and vacancy['salary']['from'] else '0'} - "
                f"{vacancy['salary']['to'] if vacancy.get(
                                          'salary') and vacancy['salary']['to'] else '0'}",
                f"{vacancy['salary']['currency'] if vacancy.get('salary') else 'RUR'}",
...
def filter_vacancies(
    vacancies: list[Vacancy], filter_words: list, salary_range: str, currency: str, top_n: int
) -> list[Vacancy]:
    """
    ...
    for vacancy in vacancies:
        ...
        if [word for word in filter_words if word.lower() in str(vacancy.responsibility).lower().split()] != [] or [
            word for word in filter_words if word.lower() in str(vacancy.requirement).lower().split()
        ] != []:
...
def get_vacancies_by_salary(filtered_vacancies: list) -> str:
    ...
    for vacancy in sorted(
        filtered_vacancies,
        key=lambda x: (
            int(x.salary.split(" - ")[1]) if int(x.salary.split(" - ")[1]) > 0 else int(x.salary.split(" - ")[0])
        ),
        reverse=True,
    ):
